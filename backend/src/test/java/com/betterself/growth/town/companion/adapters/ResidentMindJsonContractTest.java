...
class ResidentMindJsonContractTest {
    ...
    @Test void dialogueContractCarriesPerceivedWorkStateAndParsesAnExplicitOffer() {
        ...
        var context=new ResidentMind.Context("artist","14:00","sunny",ResidentMind.actorView(self),state.goal,List.of(),List.of(),
            ResidentMind.memoryViews(w.memories.stream().filter(m->m.ownerId().equals("artist")).toList()),ResidentMind.actorViews(List.of(ResidentSimulation.actor(w,"owner"))),List.of(),List.of(new ResidentMind.KnownPlaceView("cafe","有六个独立窗边座位，适合安静工作",List.of("read","work"))),List.of(),List.of(),
            ...
            ResidentMind.planView(state.plan,now),List.of(arrangement),state.occupation,List.of("observe","rest"),"owner",List.of("我是咖啡馆当前经营者"),false,List.of(new ResidentMind.ServiceRequestView("drink-7","student","water","waiting","cafe")),"open",null,null,null,null);
