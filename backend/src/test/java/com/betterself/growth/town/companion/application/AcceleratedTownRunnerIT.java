...
class AcceleratedTownRunnerIT {
    @Test void acceleratedRunProducesAReadableTimeline() throws Exception {
        ...
        System.out.println("[accelerated-run] starting: days=" + days + " modelEnabled=" + modelEnabled + " outDir=" + outDir.toAbsolutePath());
        ...
        assertThat(Files.exists(outDir.resolve("blind-test/quiz.txt"))).isTrue();
        assertThat(Files.exists(outDir.resolve("blind-test/answer-key.json"))).isTrue();
