...
public class TownConfidantService {
    ...
    public TownConfidantService(JdbcTemplate jdbc, PublicIdGenerator ids, Clock clock, TownLetterService letters,
                                ...
                                org.springframework.transaction.support.TransactionTemplate tx, TownDailyProduction daily) {
        this.jdbc = jdbc;
        this.ids = ids;
        this.clock = clock;
    ...
    public void write(long userId, String message) {
        ...
        jdbc.update(
            """
                ...
                """,
            ids.next(), userId, trimmed, Timestamp.from(clock.instant())
        );
    }

    /** Generate replies outside transactions, then atomically claim each OUT row and deliver
     * both copies. Concurrent jobs may generate twice, but can commit only one reply per letter. */
    public void runNightly(long userId, LocalDate localDate) {
        ...
        for (PendingLetter item : pending) {
            ...
            tx.executeWithoutResult(status -> {
                // A conditional OUT-row claim and both delivery copies commit or roll back together.
                int claimed = jdbc.update("update town_confidant_thread set answered_at=? where id=? and user_id=? and direction='OUT' and answered_at is null",
                    Timestamp.from(clock.instant()), item.id(), userId);
                if (claimed == 0) return;
                jdbc.update("""
                    insert into town_confidant_thread (public_id,user_id,direction,body,written_at,deliver_at,read_at,reply_to_id)
                    values (?,?,'IN',?,?,?,null,?)
                    """, ids.next(),userId,reply,Timestamp.from(clock.instant()),Timestamp.from(clock.instant()),item.id());
                letters.deliver(userId, "CONFIDANT", null, "LONG", reply, now);
            });
        }
    }

    /** 写于用户 {@code localDate} 当天开始之前、且尚未被逐封认领的来信——按写信时间从早到晚处理。 */
    private List<PendingLetter> unansweredBefore(long userId, LocalDate localDate) {
        return jdbc.query(
            ...
            userId, Timestamp.from(localDate.atStartOfDay(daily.zone(userId)).toInstant())
        );
    }
