...
describe('Journal entry and editing', () => {
  const goal = { publicId: 'g1', title: '读书', startDate: '2020-01-01', endDate: '2099-12-31', status: 'ACTIVE' }
  beforeEach(() => {
    api.get.mockReset().mockImplementation((path: string) => Promise.resolve(path.startsWith('/task-schedules')
      ? [{ publicId: 's1', taskPublicId: 't1', taskTitle: '原来的标题', plannedStartAt: new Date().toISOString(), status: 'PLANNED' }]
      : path.startsWith('/goals') ? [goal] : null))
    api.post.mockReset().mockResolvedValue({})
  })
  const render = () => mount(TodayView, { global: { stubs: { Teleport: true, RouterLink: { template: '<a><slot /></a>' } } } })

  it('creates one dated task, keeps failed text, and clears it only after saving', async () => {
    const wrapper = render()
    await flushPromises()
    await wrapper.get('[aria-label="新事项"]').setValue('给自己读十页书')
    api.post.mockRejectedValueOnce(new Error('offline'))
    await wrapper.get('.quick-entry').trigger('submit')
    await flushPromises()
    expect((wrapper.get('[aria-label="新事项"]').element as HTMLInputElement).value).toBe('给自己读十页书')
    expect(wrapper.get('[role="alert"]').text()).toContain('文字已保留')
    api.post.mockResolvedValueOnce({})
    await wrapper.get('.quick-entry').trigger('submit')
    await flushPromises()
    expect(api.post).toHaveBeenLastCalledWith('/tasks', expect.objectContaining({ goalPublicId: 'g1', title: '给自己读十页书', rrule: null, estimatedMinutes: 15 }))
    expect((wrapper.get('[aria-label="新事项"]').element as HTMLInputElement).value).toBe('')
    wrapper.unmount()
  })

  it('loads a past page and submits the entry for that date', async () => {
    const wrapper = render()
    await flushPromises()
    await wrapper.get('[aria-label="手账日期"]').setValue('2026-01-15')
    await flushPromises()
    expect(api.get).toHaveBeenCalledWith('/task-schedules?localDate=2026-01-15')
    expect(wrapper.text()).toContain('翻看与补记')
    await wrapper.get('[aria-label="新事项"]').setValue('补记阅读')
    await wrapper.get('.quick-entry').trigger('submit')
    await flushPromises()
    expect(api.post).toHaveBeenCalledWith('/tasks', expect.objectContaining({ activeFrom: '2026-01-15', activeUntil: '2026-01-15' }))
    wrapper.unmount()
  })
})
