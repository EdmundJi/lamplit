...
describe('town-npc.types', () => {
  ...
  it('accepts a fully-formed dayPlan matching CONTRACT-M7.md §1\'s sample payload', () => {
    const origins: NpcErrandOrigin[] = ['RHYTHM', 'DEVIATION', 'EVENT']
    expect(origins).toHaveLength(3)

    const errand: NpcErrand = { place: 'gym', activity: 'idle', startMinute: 420, endMinute: 480, priority: 2, origin: 'RHYTHM' }
    const leg: NpcLeg = { fromPlace: 'home', toPlace: 'gym', departMinute: 405, arriveMinute: 420 }
    const dayPlan: NpcDayPlan = { date: '2026-09-06', errands: [errand], legs: [leg] }

    expect(dayPlan.errands[0].priority).toBeGreaterThanOrEqual(0)
    expect(dayPlan.errands[0].priority).toBeLessThanOrEqual(2)
    expect(dayPlan.legs[0].arriveMinute).toBe(dayPlan.errands[0].startMinute)
  })

  it('TownNpcView.dayPlan is optional — an M7-less backend payload still type-checks', () => {
    const npc: TownNpcView = {
      code: 'GUIDE',
      displayName: '小助',
      layer: 1,
      ...
      schedule: [{ startHour: 0, endHour: 24, place: 'academy', activity: 'idle' }],
      talkingPoints: [],
      // no `dayPlan` here on purpose
    }
    expect(npc.dayPlan).toBeUndefined()
  })
})
