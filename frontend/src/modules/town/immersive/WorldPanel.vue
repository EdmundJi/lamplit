...
<style scoped>
.world-panel { position: fixed; display: flex; flex-direction: column; max-height: min(78vh, 640px); border: 1px solid color-mix(in srgb, var(--border) 60%, transparent); border-radius: var(--radius-panel); background: color-mix(in srgb, var(--surface) 94%, transparent); backdrop-filter: blur(14px); box-shadow: var(--shadow); overflow: hidden; }
.world-panel.is-compact { width: min(360px, calc(100vw - 24px)); }
.world-panel.is-wide { width: min(560px, calc(100vw - 24px)); }
.world-panel-head { display: flex; align-items: center; gap: 10px; padding: 12px 8px 12px 14px; border-bottom: 1px solid var(--border); background: var(--surface-muted); cursor: grab; touch-action: none; }
...
.world-panel-body { min-height: 80px; overflow: auto; }
...
@media (prefers-reduced-motion: reduce) { .world-panel { transition: none; } }
@media (max-width: 760px) {
  .world-panel,
  .world-panel.is-compact,
  .world-panel.is-wide { left: 0 !important; right: 0; bottom: 0; top: auto !important; width: 100%; max-height: 72vh; border-radius: var(--radius-panel) var(--radius-panel) 0 0; }
}
</style>
