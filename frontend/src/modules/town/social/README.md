...
LONG / NOTE / INVITE 分别为树洞长信、居民短笺、活动请柬，均使用同一阅读与标读流程。M10 新 INVITE 增加可空 `eventPublicId`，展开请柬后通过 `/town/events/{id}` 加载具体活动，支持参加意向、略过和明确记录参加；历史请柬保持空关联，不从正文猜测时间、地点或身份。活动公告由 `TownEventsBoard` 提供当天与过去七天入口，只有当天列表进入世界渲染。
...
`FriendsPanel` 增加「好友拜访」，展示 `TownVisits`：本人主动设置分享、已接受好友精选小景、幂等明信片及本人收件箱。默认关闭分享，不提供实时位置或私密任务。关闭/最小化信箱会卸载私密内容；当天任务、目标、AI 输入可在窗口切换时保留。

信箱标签页还提供主动展开的邮递员故事，未展开不查询；普通对话同样提供故事入口。新接口与迁移见 `docs/archive/2026-09-07/成长小镇-产品体验完善.md`。
