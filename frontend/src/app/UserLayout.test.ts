...
describe('UserLayout', () => {
  ...
  it('keeps every desktop destination available from the mobile navigation', async () => {
    ...
    expect(desktopLabels).toEqual(['今日', '目标', '伙伴', '好友', '属性', '洞察', 'AI 助手', '个人', '设置'])
    expect(wrapper.findAll('.mobile-nav a').map(link => link.text())).toEqual(['今日', '目标', '属性', 'AI 助手', '个人'])

    await wrapper.get('.mobile-nav button').trigger('click')

    expect(wrapper.findAll('.mobile-more-links a').map(link => link.text())).toEqual(['伙伴', '好友', '洞察', '设置'])
  })
