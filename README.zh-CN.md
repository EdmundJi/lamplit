...
现在的入口是 `/town`：搬进一个有 25 名居民、6 处公共去处、分房间住宅的小镇。
物件可以被占用、磨损、修理、借出或赠送；居民只知道自己亲眼看见或听说的内容。
用户可以选择真实 Todo 陪伴专注，也可以让小人按自己的节律生活。世界由后端保存，刷新后继续；专注结束不会自动完成真实任务。

居民保留自己的需求、计划、关系和有来源的记忆，实际相遇会带来协商、合作及新的小愿望。模型可以在规则约束内调整计划和提出新项目；服务不可用时，居民仍按已有计划生活。认证和真实任务沿用现有基础，新小镇存档和推进逻辑独立。

小镇默认只用 Qwen，固定在带日期的快照 `qwen3.7-flash-2026-07-15`（实验要控制变量）；DeepSeek 只有显式配置路由时才会用；每次实际响应会记录供应商和模型名。
密钥只放在被忽略的 `.env.local`。无模型或模型失败时，规则只继续已有合法安排或中性等待，不会替居民编一句话。
