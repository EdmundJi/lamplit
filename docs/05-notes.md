...
| --- | --- | --- | --- |
| qwen3.8-flash + `enable_thinking:false` | **1.01s** | **22** | 0 |
| qwen3.8-flash 默认 | 3.70s | 235 | 219 |
| deepseek-v4-flash 默认 | 1.69s | 76 | 53 |
| deepseek + `enable_thinking:false` | 2.06s | 55 | 42（**不认这个参数**） |

三条结论：

**关掉思考是数量级的差别，换供应商不是。** Qwen 关思考是全场最快最省，不关是全场最差——差 3.7 倍耗时、10 倍输出 token。

**不加这个参数直接换 Qwen 是明显退步**，端到端会比 DeepSeek 慢一倍以上。这是接第二个供应商时最大的坑。

**"DeepSeek 啰嗦"这个说法在这类提示词上不成立。** 它可见的回答就是一句话，和别人一样长；76 个输出 token 里 53 个是隐藏的推理 token。token 没花在正文上，所以"它给你写三段分析"不是这里发生的事。DeepSeek 也关不掉推理（试了 `enable_thinking:false`，推理 token 只从 53 降到 42，是噪声）。
